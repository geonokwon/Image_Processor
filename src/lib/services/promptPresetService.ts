...
if (typeof window !== 'undefined') {
  throw new Error('promptPresetService는 서버에서만 사용할 수 있습니다');
}
...
export class PromptPresetService {
  ...
  constructor() {
    this.presetFilePath = path.join(process.cwd(), 'data', 'prompt-presets.json');
  }
  ...
  async getPresets(): Promise<any[]> {
    ...
    try {
      ...
      logger.error(`프롬프트 프리셋 불러오기 실패: ${error.message}`, error);
      throw new Error(`프롬프트 프리셋 불러오기 실패: ${error.message}`);
    }
  }
  ...
  async savePresets(presets: any[]): Promise<void> {
    ...
    try {
      ...
      logger.info('프롬프트 프리셋 저장 성공');
    } catch (error: any) {
      logger.error(`프롬프트 프리셋 저장 실패: ${error.message}`, error);
      throw new Error(`프롬프트 프리셋 저장 실패: ${error.message}`);
    }
  }
}
